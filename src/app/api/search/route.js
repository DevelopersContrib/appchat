...
export async function GET(request) {
  ...
  const m = await requireMembership(sp.get('tenant'), user.id).catch(() => null);
  if (!m) return NextResponse.json({ error: 'Not a member' }, { status: 403 });

  const q = (sp.get('q') || '').trim().slice(0, 200);
  const type = sp.get('type') === 'files' ? 'files' : 'messages';
  const channelId = Number(sp.get('channelId')) || null;
  const fromId = Number(sp.get('fromId')) || null;
  const recent = sp.get('sort') === 'recent';
  if (q.length < 2 && !fromId && !(type === 'files' && channelId)) return NextResponse.json({ results: [] });
